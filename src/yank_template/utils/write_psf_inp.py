...
def write_psf_inp(crd_infile, psf_outfile, mol_segid_list, mol_n_list):
    output_file = "psf.inp"
    ...
    with open(output_file, 'w') as inp:
        ...
        inp.write("! Write the CHARMM-format PSF file\n")
        inp.write(f"open write unit 40 card name {psf_outfile}\n")
        inp.write("write psf card unit 40\n")
        inp.write("close unit 40\n")
        inp.write("\n")
        inp.write("stop")
        inp.close()
        logger.debug("Wrote psf inp")
    return(f"Wrote inp for psf")
